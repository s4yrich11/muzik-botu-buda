...
@bot.command(name='play', help='İsimle aratarak veya link yapıştırarak müzik çalar.')
async def play(ctx, *, search: str):
    ...
    async with ctx.typing():
        # Eğer girilen ifade bir link değilse SoundCloud'da arat, link ise doğrudan aç
        ...
        try:
            ...
            print(f"Veri çekme hatası (yt-dlp): {e}")
...
@bot.command(name='skip', help='Çalan müziği atlar / geçer.')
async def skip(ctx):
    if ctx.voice_client and ctx.voice_client.is_playing():
        ctx.voice_client.stop()
        await ctx.send("Müzik atlandı!")
    else:
